...
raw_cychosz_data<- read_csv("Data/meta_answers_global_CPossling_9-20.csv")
...
other_data<- read_excel("Data/CP_by_child-updated_21_01.xlsx")
other_data_tsi<- read_excel("Data/CP_by_child_tsi.xlsx")
...
mydata_sub<-subset(mydata_sub, !is.na(CP)) #NA in CP are excluded -> 72 children
...
"Average Mad_C"; cor.test(mydata_sub_A$age,mydata_sub_A$CP) #0.587497 
...
summary(mydata_sub)
"Average Mad_VQ"; cor.test(mydata_sub_Av$age,mydata_sub_Av$CP,)  #0.4773039
"Large Mad_VQ"; cor.test(mydata_sub_Lr$age,mydata_sub_Lr$CP)  #0.7266499
...
par(mfrow=c(2,2))
plot(lm_data_C)  
gvlma(lm_data_C) #Global Stat and Heteroscedasticity not satisfied
...
par(mfrow=c(2,2))
plot(lm_data_V)  
gvlma(lm_data_V) # Heteroscedasticity not satisfied
...
Comments:
* GOG_000302: CP value is 0. Should we omit this child in this case?
* For Tsimane children (from OSF) now we have 2 recordings -> a mean value of them is needed
** I get - 16117 parsing failures when try to run raw_cychosz_data<- read_csv("Data/meta_answers_global_CPossling_9-20.csv")
** My result is 63 children with all data (both phonetic and language complexity)
