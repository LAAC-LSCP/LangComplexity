...
mydata_sub<-subset(mydata_sub, !is.na(Mad_C)) #NA are excluded -> 75 children
...
#Consonants
plot(mod_complexC) #ERROR: some NaN appears
gvlma(mod_complexC) #ERROR
#Vowels
plot(mod_complexV) #ERROR: some NaN appears
...
anova(mod_complexC,mod_simpleC) #doesn't seem to be significant :(, probably data are not enough
#Vowels
mod_simpleV=lm(CR~age+Mad_VQ,data=mydata_sub) 
anova(mod_complexV,mod_simpleV) # a slight significance -  0.01 '*' so far this is the best result


# check whether it's simply due to age polynomials
mod_age=lm(CR~age+age2+age3,data=mydata_sub)
anova(mod_complexC,mod_age) #Consonants no significance
anova(mod_complexV,mod_age) #Vowels # a slight significance -  0.001 '**' so far this is the best result
...
anova(mod_complexV,mod_intV) #0.05 '.'


# add back polynomials but without interaction
#Consonants 
mod_int_ageC=lm(CR~age*Mad_C+age2+age3,data=mydata_sub)
anova(mod_complexC,mod_int_ageC) #no significance
#Vowels
mod_int_ageV=lm(CR~age*Mad_VQ+age2+age3,data=mydata_sub)
anova(mod_complexV,mod_int_ageV) # a slight significance -  0.01 '*' so far this is the best result
...
anova(mod_complexC,mod_int_age2_intC) #an interaction age3*sylcomp doesn't change anything
#Vowels
mod_int_age2_intV=lm(CR~age*Mad_VQ+age2*Mad_VQ,data=mydata_sub)
anova(mod_complexV,mod_int_age2_intV) # the interaction gives the same significance 0.01 '*'


#check for assumptions in the last model  
#Consonants 
plot(mod_int_age2_intC) 
gvlma(mod_int_age2_intC)  #satisfied
#Vowels
plot(mod_int_age2_intV) 
gvlma(mod_int_age2_intV)  #Link Function is not satisfied
plot(mod_int_ageV) 
...
#Consonants 
Anova(mod_int_age2_intC, type="III") #not very impressive
...
gvlma(mod_int_age_noTsiV) #Link Function is not satisfied
...
gvlma(mod_int_age_noTsi_ageScaledV)  #same problem with Link Function
...
gvlma(mod_int_age_noTsi_ageScaled_no_oldV) #Link Funktion is not satisfied
...
"Moderately Small Mad_C"; cor.test(mydata_sub_S$age,mydata_sub_S$CR) #0.9008206 
"Average Mad_C"; cor.test(mydata_sub_A$age,mydata_sub_A$CR) #0.6029925
...
"Average Mad_VQ"; cor.test(mydata_sub_Av$age,mydata_sub_Av$CR)  #0.6447247
"Large Mad_VQ"; cor.test(mydata_sub_Lr$age,mydata_sub_Lr$CR)  #0.7149718 
